...
import BuyEth from "./MintBtns/MintWithEth";
import BuyYen from "./MintBtns/MintWithYen";
...
export default function NftMinter({
    ...
    const {
        ...
        isSuccessTxMint,
        isErrorTxMint
    } = useNftContract({ tokenId, nftTokenPrice, totalAllowance, isMinted })
    ...
    } = useAllowance({ tokenId, nftTokenPrice, totalAllowance, isMinted })
    ...
    useEffect(() => {
        ...
        if (errorTokenMint || isErrorTxMintWithToken) toast.warn(`Mint with Token error. Try again or contact support:  ${errorTokenMint}`)
    }, [isError, errorTokenMint, isErrorTxMintWithToken])
                                ...
                                {(isDisconnected || userAddress === '' || userAddress.length < 10) ?
                                    ...
                                    <>
                                        {nftPrice !== null && nftTokenPrice !== null &&
                                            (
                                                <>
                                                    <BuyEth nftPrice={nftPrice} mint={mint} isLoading={isLoading} loadingTxMint={loadingTxMint} />
                                                    <BuyYen
                                                        loadingTxMint={loadingTxMint}
                                                        ...
                                                        submitTxAllowanceLoading={submitTxAllowanceLoading}
                                                        loadingTxMintWithToken={loadingTxMintWithToken}
                                                    />
                                                </>
                                            )
                                        }
